...
import { getMobileVideoUrl, getPosterUrl, useIsMobileViewport } from '../../components/useResponsiveVideo';
              ...
              <div className="flex md:flex-col gap-2 overflow-y-auto max-h-[85vh] flex-shrink-0">
                {productMedia.map((media, index) => (
                  ...
                  >
                    {media.type === 'video' ? (
                      <>
                        {/* Show the poster image for the thumbnail — much
                            cheaper than loading metadata for every video. */}
                        {(() => {
                          const poster = getPosterUrl(media.src);
                          return poster ? (
                            <Image
                              src={poster}
              ...
              <div className="flex-1 relative max-h-[85vh]">
                {productMedia[selectedImage]?.type === 'video' ? (
                  <video
                    ...
                    src={isMobile ? getMobileVideoUrl(productMedia[selectedImage].src) : productMedia[selectedImage].src}
                    poster={getPosterUrl(productMedia[selectedImage].src)}
                    autoPlay
                    loop
                    muted
                    playsInline
                ...
                <div className="border-t">
                  ...
                  {openDropdown === 'shipping' && (
                    <div className="pb-4 text-sm text-gray-700 space-y-2">
                      <p>Free shipping on all orders PAN India.</p>
